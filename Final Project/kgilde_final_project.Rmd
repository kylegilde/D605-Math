...
packages <- c("prettydoc",'dplyr', 'psych', 'knitr', 'ggplot2', 'MASS', 'car', 'MLmetrics', 'gvlma', 'broom', 'randomForest', 'rpart') 
...
train_df <- train_csv %>% 
  mutate(
    ...
    #change some factors back to integers
...
test_df <- test_csv %>% 
  ...
  #change some factors back to integers
  mutate_at(vars(MasVnrArea, LotFrontage, BsmtUnfSF, TotalBsmtSF, 
                 BsmtFinSF1, GarageArea, BsmtFinSF2),
                  as.integer
            ) %>% 
  dplyr::select(-c(YearBuilt, YearRemodAdd, GarageYrBlt))
...
inspect_metadata <- function(df){
  ###Takes a data frame &
  ###Checks for NAs, incorrect class types, and inspects the unique values
  df_len <- nrow(df)
...
df_summary <- inspect_metadata(train_df)
df_summary <- inspect_metadata(test_df)
View(df_summary)
...
model_summary <- function(model, model_sum, metrics_glance){
  ###Takes as inputs some of the model outputs
  ###& creates data frame of summary statitics
  results <- data.frame(
    n_vars = length(names(model$model)),
    RootMSLE = RMSLE(model$fitted.values, model$model$SalePrice),
    adj.r.sq = model_sum$adj.r.squared,
    f.statistic.and.dfs = paste(round(model_sum$fstatistic, 4), collapse = ", "),
...
if(!exists("AIC_model")) AIC_model <- stepAIC(model1)
```
...
x_train_df <- subset(train_df, select = -c(SalePrice, Id))

#fit <- rpart(y_train ~ ., data = x_train_df, method="class")
fit <- randomForest(y_train ~ ., x_train_df, ntree=500)
