...
AIC_model_PT_fit <- predict(AIC_model_PT, newdata = subset(test_df, select = cols_needed))
summary(AIC_model_PT_fit)
...
rf_fit <- randomForest(x_train_df, y = y_train, xtest = test_df, ntree=500, importance = T)
nrow(test_df)
sum(is.na(rf_fit))
# 
summary(rf_fit)
