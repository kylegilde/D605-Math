...
upper_tri_order <- function(aug_matrix){
  new_matrix <- aug_matrix[order(-aug_matrix[, 1], -aug_matrix[, 2], -aug_matrix[, 3]), ]
  return(new_matrix)
}
...
substract_rows <- function(the_matrix, row_num, the_multiplier, pivot_pos){
  first_row <- the_matrix[row_num, ]
  second_row <- the_matrix[row_num + 1, ] * the_multiplier
...
row_num <- 1
A_aug
first_row <- A_aug[row_num, ]
second_row <- A_aug[row_num + 1, ] * the_multiplier
new_row <- first_row - second_row
(A_aug[c(row_num), ] <- new_row)
...
#while (!upper_tri_test(current_matrix)){
  #for (r in 1:1){ #nrow(current_matrix)){
    r <- 1
    current_row <- current_matrix[r, ]
    pivot_pos <- determine_pivot_pos(current_row)
    the_multiplier <- determine_multiplier(current_matrix, r, pivot_pos)
    new_matrix <- substract_rows(current_matrix, r, the_multiplier, pivot_pos) 
    new_matrix
